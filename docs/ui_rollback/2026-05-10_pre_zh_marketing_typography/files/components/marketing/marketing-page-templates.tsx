...
export function HomepageTemplate(props: HomepageTemplateProps & TemplateRuntimeProps) {
  ...
  const intentVisualItems = createIndexedVisualItems(
    (props.intentColumns ?? []).slice(0, 3).map((item) => ({
      title: item.title,
      meta: item.actionLabel,
    }))
  )
  ...
  return (
    <>
      ...
      {props.intentColumns?.length ? (
        <MarketingSurface className="border-border/45 border-t py-16 sm:py-20">
          <MarketingContainer>
            <MarketingNarrativeSection
              eyebrow={props.intentEyebrow ?? 'Best fit'}
              title={props.intentTitle ?? 'Use the homepage as the public decision hub.'}
              description={
                props.intentDescription ??
                'This section helps visitors understand whether they should stay on the overview, open a core workflow, or inspect a sample report first.'
              }
              layoutVariant="offset"
              aside={
                <MarketingSectionAside
                  eyebrow={props.intentEyebrow ?? 'Best fit'}
                  items={intentVisualItems}
                  tone="default"
                />
              }
            >
              <MarketingBalancedFeatureGrid
                pageKey={props.pageKey}
                locale={props.locale}
                items={props.intentColumns}
                icon={<FileSearch className="size-4 text-[oklch(0.74_0.15_74)]" />}
                variant="editorial"
              />
            </MarketingNarrativeSection>
          </MarketingContainer>
      ...
      <MarketingCtaBanner pageKey={props.pageKey} locale={props.locale} closing={props.closing} />
    </>
  )
}
      ...
      {props.intentColumns?.length ? (
        <MarketingSurface className="border-border/45 border-t py-16 sm:py-20">
          <MarketingContainer>
            <MarketingNarrativeSection
              eyebrow={props.intentEyebrow ?? 'Best fit'}
              title={props.intentTitle ?? 'Use this page when the workflow fits your next decision.'}
              description={
                props.intentDescription ??
                'This section helps search visitors quickly decide whether this page is the right workflow or whether they should switch to another page first.'
              }
              layoutVariant="offset"
              aside={
                <MarketingSectionAside
    ...
    <>
      ...
      {props.intentColumns?.length ? (
        <MarketingSurface className="border-border/45 border-t py-16 sm:py-20">
          <MarketingContainer>
            <MarketingNarrativeSection
              eyebrow={props.intentEyebrow ?? 'How to use this sample'}
              title={props.intentTitle ?? 'Use the sample report to judge output quality before login.'}
              description={
                props.intentDescription ??
                'This section helps visitors decide whether the sample matches their market shape and what they should learn from it before running their own analysis.'
              }
              layoutVariant="offset"
              aside={
                <MarketingSectionAside
